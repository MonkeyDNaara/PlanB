...
const EventDetails = () => {
  ...
  return (
    <main className="relative mx-auto flex min-h-[calc(100svh-9rem)] w-full max-w-6xl items-center px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
      <article className="relative w-full overflow-hidden rounded-[2rem] border border-white/15 bg-[#080914]/85 shadow-[0_24px_90px_rgba(0,0,0,0.55),0_0_60px_rgba(124,58,237,0.12)] backdrop-blur-2xl">
        <div
          className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_15%_0%,rgba(139,92,246,0.18),transparent_38%),linear-gradient(135deg,rgba(255,255,255,0.06),transparent_42%)]"
          aria-hidden="true"
        />
        ...
        <div className="relative p-5 sm:p-8 lg:p-10">
          <div className="flex flex-col gap-4 border-b border-white/10 pb-5 sm:flex-row sm:items-center sm:justify-between">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="inline-flex w-fit items-center rounded-full border border-white/15 bg-white/8 px-4 py-2.5 text-sm font-bold text-white transition hover:border-violet-300/55 hover:bg-violet-500/20 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-300"
            >
              ← Zurück
            </button>
          ...
          <div className="pt-7 sm:pt-9">
            <span className="inline-flex rounded-full border border-violet-300/30 bg-violet-500/15 px-3 py-1 text-[0.68rem] font-black uppercase tracking-[0.24em] text-violet-200">
              Eventdetails
            </span>

            <h1 className="mt-4 max-w-4xl text-pretty text-3xl font-black leading-tight text-white sm:text-4xl lg:text-5xl">
              {title}
            </h1>

            <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-[0.85fr_0.7fr_1.8fr]">
              <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
                <span className="block text-[0.65rem] font-black uppercase tracking-[0.2em] text-violet-300">
                  Datum
                </span>
              ...
              <div className="min-w-0 rounded-2xl border border-white/10 bg-black/25 p-4">
                ...
                <strong className="mt-2 block break-words text-base text-white sm:text-lg">
            ...
            {description && (
              <section className="mt-6 rounded-2xl border border-white/10 bg-black/20 p-5 sm:p-6">
                <h2 className="text-xs font-black uppercase tracking-[0.2em] text-violet-300">
                  Über dieses Event
                </h2>
                <p className="mt-3 max-w-4xl whitespace-pre-line text-sm leading-7 text-slate-200 sm:text-base">
                  {description}
                </p>
              </section>
            )}
